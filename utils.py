...
class GLUE_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        if len(self.data_cols) == 1: 
            ...
            tok_output = self.tokenizer(text=tok_input, padding="max_length", truncation=True, max_length=128, return_tensors="pt")
        else: 
            tok_input = (self.data_df[self.data_cols[0]].iloc[index], self.data_df[self.data_cols[1]].iloc[index])
            tok_output = self.tokenizer(*tok_input, padding="max_length", truncation=True, max_length=128, return_tensors="pt")
...
def get_data_df(conf, train=False):
    ...
    if conf['task_folder'] == 'RTE':
        df['bin_label'] = (df['label'] != 'entailment') * 1.
    return df
